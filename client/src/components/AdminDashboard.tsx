...
export function AdminDashboard({ projects, evaluations, onNavigate, onViewProject }: AdminDashboardProps) {
  ...
  const statCards = [
    ...
    { label: "Total Projects", value: stats.totalProjects, icon: BookOpen, color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Open Positions", value: stats.openPositions, icon: Briefcase, color: "text-purple-600", bg: "bg-purple-50" },
    { label: "Rooms Booked Today", value: stats.roomsBooked, icon: Calendar, color: "text-orange-600", bg: "bg-orange-50" },
  ];
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-slate-300">
              {stats.pendingApprovals} pending approval{stats.pendingApprovals !== 1 ? "s" : ""} · {pendingEvals.length} evaluations awaiting review
            </p>
          </div>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {[
              ...
              { label: "Faculty", count: stats.totalFaculty, color: "bg-emerald-500" },
              { label: "Admins", count: stats.totalUsers > 0 ? stats.totalUsers - stats.totalStudents - stats.totalFaculty : 0, color: "bg-slate-500" },
            ].map((item) => (
          ...
          <div className="space-y-3">
            {stats.recentActivity.map((item) => (
              <div key={item.id} className="flex items-start gap-3">
                <div className={`mt-1.5 size-2 rounded-full shrink-0 ${activityColor[item.type] || "bg-gray-400"}`} />
